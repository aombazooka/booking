 ...
 */
function acquireBookingLock(PDO $pdo, string $date, ?int $staffId): void
{
    $key = 'bk:' . $date . ':' . ($staffId ?? 0);
 ...
 */
function findTimeConflicts(PDO $pdo, string $date, string $start, string $end, ?int $staffId = null, ?int $excludeId = null): array
{
    $sql = "
 ...
 */
function resolveStaffId(PDO $pdo, $raw, bool $activeOnly = false): ?int
{
    ...
    $sql = "SELECT id FROM staff WHERE id = ?" . ($activeOnly ? " AND is_active = 1" : "");
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$id]);
 ...
 */
function upsertCustomer(PDO $pdo, string $name, string $phone): ?int
{
    $phone = trim($phone);
    $name = trim($name);
    if ($phone === '') {
